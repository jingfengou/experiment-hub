#!/usr/bin/env bash
# Interleave reasoner for rotation baseline（无步骤提示，仅题干图像）
...
DATA_JSON=${DATA_JSON:-data_handlers/rotation_interleave/base/rotation_base.jsonl}
DATA_ROOT=${DATA_ROOT:-data_handlers/rotation_interleave/base}
OUTPUT_DIR=${OUTPUT_DIR:-outputs/rotation_mathcanvas_interleave_base}
MAX_SAMPLES=${MAX_SAMPLES:-1000}

if [ ! -f "$DATA_JSON" ]; then
  echo "Preparing rotation base jsonl..."
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=23456 \
  ...
  --enable_taylorseer false | tee logs/rotation_mathcanvas_ddp.log
