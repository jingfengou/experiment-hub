#!/usr/bin/env bash
# Interleave reasoner for rotation with step hints（去掉最终步骤图像）
...
DATA_JSON=${DATA_JSON:-data_handlers/rotation_interleave/nofinal/rotation_nofinal.jsonl}
DATA_ROOT=${DATA_ROOT:-data_handlers/rotation_interleave/nofinal}
OUTPUT_DIR=${OUTPUT_DIR:-outputs/rotation_mathcanvas_interleave_nofinal}
MAX_SAMPLES=${MAX_SAMPLES:-1000}

if [ ! -f "$DATA_JSON" ]; then
  echo "Preparing rotation nofinal jsonl..."
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=23458 \
