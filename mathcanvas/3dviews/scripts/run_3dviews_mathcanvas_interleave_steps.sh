#!/usr/bin/env bash
# Interleave reasoner for 3Dviews（含步骤提示，屏蔽选项泄露）
...
DATA_JSON=${DATA_JSON:-data_handlers/3dviews_interleave/steps/3dviews_steps.jsonl}
DATA_ROOT=${DATA_ROOT:-data_handlers/3dviews_interleave/steps}
OUTPUT_DIR=${OUTPUT_DIR:-outputs/3dviews_interleave_steps}
MAX_SAMPLES=${MAX_SAMPLES:-0}

if [ ! -f "$DATA_JSON" ]; then
  echo "Preparing 3Dviews steps jsonl..."
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=23456 \
  ...
  --enable_taylorseer false | tee logs/3dviews_interleave_steps.log
