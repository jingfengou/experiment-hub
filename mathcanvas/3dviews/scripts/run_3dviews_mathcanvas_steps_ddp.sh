#!/usr/bin/env bash
# 非图文交替：带中间过程（文本+步骤图，屏蔽选项分析）DDP 推理
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/3dviews_mathcanvas_steps_ddp}
...
torchrun --nproc_per_node=$NUM_GPUS --master_port=23458 \
  scripts/inference/infer_3dviews_mathcanvas_steps_ddp.py \
  --model-dir "$MODEL_DIR" \
  --ckpt-dir "$CKPT_DIR" \
  --ckpt-file "$CKPT_FILE" \
  ...
  --sample-fraction 1.0 \
  --sample-region head \
  --text-temperature 0.3 | tee logs/3dviews_mathcanvas_steps_ddp.log
