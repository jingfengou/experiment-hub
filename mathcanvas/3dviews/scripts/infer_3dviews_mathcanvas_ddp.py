#!/usr/bin/env python3
"""
Two-GPU (DDP) inference on the 3Dviews thinking-chain dataset (no step hints).
This reuses the rotation baseline prompt format for pure understanding.
"""
import argparse
import json
import math
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="3Dviews MathCanvas inference (DDP, no steps).")
    ...
    parser.add_argument("--output-dir", default="./outputs/3dviews_mathcanvas_ddp", help="Output directory for generated texts.")
...
def resolve_image(dataset_root: str, item: dict) -> Image.Image:
    ...
    # Fallback: try using Image_id folder directly
...
def main() -> None:
    ...
    for idx in assigned:
        ...
        image = resolve_image(args.dataset_root, item)
